...
def add_alpha_channel(img):
    ...
    alpha_channel = np.ones(b_channel.shape, dtype=b_channel.dtype) *255 # 创建Alpha通道    255表示透明
...
transformer1=transforms.Compose([
    ...
    transforms.RandomCrop((300,300),padding=25,fill=125,padding_mode="edge")
])

transformer2=nn.ModuleList([
...
if __name__ == '__main__':
    ...
    with open('./train_trans/label.csv',"w", newline='') as csvfile:
    ...
    for ct, name in enumerate(files):
        while 1:
            ...
            img2 = transformer1(img_png)
            img2 = transforms.RandomApply(transformer2, p=0.5)(img2)
            img_png = np.array(img2)
        ...
        cv2.imwrite('./train_trans/images/' + name.split('/')[-1], res_img)
        with open('./train_trans/label.csv', "a", newline='') as csvfile:
            writer = csv.writer(csvfile)
